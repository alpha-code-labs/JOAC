...
public class UIPathDragger : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler
{
    ...
    private const float LINE_SPACING = 15f; // Minimum distance between lines
    ...
    void Start()
    {
        ...
        minX = startPos.x - 200f; // Adjust range based on UI scale
        maxX = startPos.x + 200f;
    }
    ...
    void Update()
    {
        ...
        if (uiManager != null && uiManager.playerPositionSliderValue != null)
        {
            float move_X = uiManager.playerPositionSliderValue;
    ...
    public void OnDrag(PointerEventData eventData)
    {
        ...
        isAtMaxDrag = (dragCoefficient >= 150f);

        UpdatePath(startPos, newPos);

        float score = dragCoefficient / 150f;
    ...
    void UpdatePath(Vector2 from, Vector2 to)
    {
        ...
        for (int i = 0; i < pathLines.Count; i++)
        {
            ...
            if (i != 0)
                lineRect.sizeDelta = new Vector2(48f + i * 2f, 48 + i * i * .1f);
            else
                lineRect.sizeDelta = new Vector2(48f, 48f);
